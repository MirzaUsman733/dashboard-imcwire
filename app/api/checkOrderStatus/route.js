...
//     // const order =  req?.headers?.get('referer')
//     // const ordIdIndex = order?.lastIndexOf('/');
//     // ordId = order?.substring(ordIdIndex + 1);
//     // console.log("Order Id",ordId)
//     const protocol = req.headers.get('x-forwarded-proto') || 'http';
//     const host = req.headers.get('host');
//     url = new URL(req.url, `${protocol}://${host}`);
//     ordId = url.searchParams.get("ordId");
//   //   const urlParams = new URLSearchParams(req?.url?.split("?")[1]); // Extract query parameters from URL
...
export async function GET(req) {
  ...
  try {
    ...
    const orderStatusResponse = await fetch(
      `${process.env.Paypro_URL}/v2/ppro/ggos?userName=${encodeURIComponent(
        "Tier_Solutions"
      )}&cpayId=${ordId}`,
      {
        method: "GET",
